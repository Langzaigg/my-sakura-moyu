...
def fix_line(line, styles_map):
    ...
    header_raw = ",".join(parts[:9])

    header_shadow = re.sub(r'^Dialogue: \s*\d+', 'Dialogue: 5', header_raw)
    
    header_shadow = re.sub(r'^Dialogue: \s*\d+', 'Dialogue: 5', header_raw)
    
    SHADOW_COLOR = '&H202020&'
    SHADOW_ALPHA = '&H80&'
    EXTRA_DIST = 0

    def transform_fshp_shadow(match):
        val = float(match.group(1))
        new_xshad = base_shad_x - val + shad_w + EXTRA_DIST
        return f"\\xshad{new_xshad:g}"

    t_shadow = re.sub(r'\\fshp([-]?\d+(\.\d+)?)', transform_fshp_shadow, original_text)
    t_shadow = re.sub(r'\\pos\([-]?\d+,[-]?\d+\)', lambda m: f"\\pos({OFFSET_X},{OFFSET_Y})", t_shadow)
    
    raw_tags_shadow = re.search(r'\{.*?\}', t_shadow).group(0)
    base_tags_shadow = clean_noise(raw_tags_shadow)
    content_shadow = t_shadow.replace(raw_tags_shadow, '')

    tags_shadow = base_tags_shadow.replace('{', f'{{{common}\\alpha{SHADOW_ALPHA}\\yshad{base_shad_y + shad_w + EXTRA_DIST:g}\\bord{style_cfg["out_w"]}\\blur3\\4c{SHADOW_COLOR}')
    line_shadow = header_shadow + "," + tags_shadow + content_shadow

    header_glow = re.sub(r'^Dialogue: \s*\d+', 'Dialogue: 6', header_raw)

    def transform_fshp_glow(match):
        val = float(match.group(1))
        new_xshad = base_shad_x - val + shad_w
        return f"\\xshad{new_xshad:g}"

    t_glow = re.sub(r'\\fshp([-]?\d+(\.\d+)?)', transform_fshp_glow, original_text)
    ...
    tags_glow = base_tags_glow.replace('{', f'{{{common}\\yshad{base_shad_y + shad_w:g}\\bord{style_cfg["out_w"]}\\blur1.5\\4c{style_cfg["shadow"]}')
    line_glow = header_glow + "," + tags_glow + content
    ...
    header_outline = re.sub(r'^Dialogue: \s*\d+', 'Dialogue: 7', header_raw)
    tags_outline = base_tags_main.replace('{', f'{{{common}\\yshad{base_shad_y:g}\\bord{style_cfg["out_w"]}\\blur0.5\\4c{style_cfg["outline"]}')
    line_outline = header_outline + "," + tags_outline + content

    header_fill = re.sub(r'^Dialogue: \s*\d+', 'Dialogue: 8', header_raw)
    tags_fill = base_tags_main.replace('{', f'{{{common}\\yshad{base_shad_y:g}\\bord0\\blur0\\4c{style_cfg["primary"]}')
    line_fill = header_fill + "," + tags_fill + content

    return [line_shadow, line_glow, line_outline, line_fill]
